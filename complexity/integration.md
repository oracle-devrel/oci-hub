---
layout: default
title: 4. Integration
parent: Complexity levels
nav_order: 4
---

# Using OCI with/from anything else
{: .no_toc }

1. TOC
{:toc}

## Languages

### Java
- [Autonomous Database with Spring Boot](https://github.com/vmleon/adbclient-java)
- [Oracle Cloud Vault as a Secure, Distributed Config Store for your Micronaut Applications](https://blogs.oracle.com/developers/oracle-cloud-vault-as-a-secure%2C-distributed-config-store-for-your-micronaut-applications)
- [Oracle Universal Connection Pool (UCP) Support Added To Micronaut](https://blogs.oracle.com/developers/oracle-universal-connection-pool-ucp-support-added-to-micronaut)
- [Building Cross Platform Native Images With GraalVM](https://blogs.oracle.com/developers/building-cross-platform-native-images-with-graalvm)
- [Cloud Cost Cutting: Save Money By Resizing Large Images In Object Storage](https://blogs.oracle.com/developers/cloud-cost-cutting%3A-save-money-by-resizing-large-images-in-object-storage)
- [Ring Central - Sending SMS Messages With Java](https://recursive.codes/blog/post/1384)
- [Ring Central - Making A Voice Call With Java](https://recursive.codes/blog/post/1302)

### JDBC
- [More options for consuming Oracle JDBC drivers from Maven Central](https://blogs.oracle.com/developers/more-options-for-consuming-oracle-jdbc-drivers-from-maven-central)
- [Connecting to Oracle Autonomous Database via JDBC](https://blogs.oracle.com/developers/connecting-to-oracle-autonomous-database-via-jdbc)
- [Configuring the Oracle JDBC drivers with Quarkus](https://blogs.oracle.com/developers/configuring-the-oracle-jdbc-drivers-with-quarkus)
- [Configuring the Oracle JDBC drivers in a SpringBoot project](https://blogs.oracle.com/developers/configuring-the-oracle-jdbc-drivers-in-a-springboot-project)

### Javascript / Node.js
- [Instance and Resource Principal Authentication With The OCI TypeScript/JavaScript SDK](https://blogs.oracle.com/developers/instance-and-resource-principal-authentication-with-the-oci-typescriptjavascript-sdk)
- [OCI SDK For TypeScript Is Now Available - Here's How To Use It In Your JavaScript Projects](https://blogs.oracle.com/developers/oci-sdk-for-typescript-is-now-available-heres-how-to-use-it)
- [Resource Principal Auth With Node.JS For Easy OCI REST API Access From Your Oracle Functions](https://blogs.oracle.com/developers/resource-principal-auth-with-nodejs-for-easy-oci-rest-api-access-from-your-oracle-functions)
- [From locally running Node application to Cloud based Kubernetes Deployment](https://medium.com/oracledevs/from-locally-running-node-application-to-cloud-based-kubernetes-deployment-1fed34280952)
- [Instance and Resource Principal Authentication With The OCI TypeScript/JavaScript SDK](https://recursive.codes/blog/post/1600)
- [Ring Central - Sending an MMS With Node.JS](https://recursive.codes/blog/post/1384)
- [Installing Node-RED In An Always Free VM On Oracle Cloud](https://recursive.codes/blog/post/112)
- [Oracle Functions - Connecting To ATP With Node.JS](https://recursive.codes/blog/post/130)
- [Creating A Microservice With Node & SODA - JSON Document Storage In Autonomous DB](https://recursive.codes/blog/post/1135)

### Python
- [Getting Started With The Oracle Cloud Infrastructure Python SDK](https://blogs.oracle.com/developers/getting-started-with-the-oracle-cloud-infrastructure-python-sdk-v2)
- [Writing Python scripts to run from the OCI Console Cloud Shell](https://medium.com/oracledevs/writing-python-scripts-to-run-from-the-oci-console-cloud-shell-a0be1091384c)
- [Deploy microservices to a Kubernetes cluster](https://docs.oracle.com/en/solutions/deploy-microservices/index.html)

### Swift (iOS/macOS)
- [Build Swift apps with Oracle’s Autonomous Database and NoSQL](https://medium.com/so-much-code/build-swift-apps-with-oracles-autonomous-database-and-nosql-f1dee7e7cec3)

### .Net
- [OCI SDK for .NET is now available for your .NET Projects](https://blogs.oracle.com/developers/oci-sdk-for-net-is-now-available-for-your-net-projects-v2)

### Go
- [Connecting to Oracle Databases using Godror and Sqlx](https://blogs.oracle.com/developers/connecting-to-oracle-databases-using-godror-and-sqlx)

### PHP
- [Installing Apache and PHP 7 on OCI](https://medium.com/oracledevs/installing-apache-and-php-7-on-oci-c0ccc2c8409d)

## Devops

### Kubernetes
- [Project Verrazzano Now on GitHub](https://blogs.oracle.com/developers/project-verrazzano-now-on-github)
- [Learn About Setting Up a Kubernetes Cluster in the Cloud](https://docs.oracle.com/en/solutions/deploy-kubernetes/#GUID-540E0F3B-6575-4303-98CF-3A05F53908EE)
- [From locally running Node application to Cloud based Kubernetes Deployment](https://medium.com/oracledevs/from-locally-running-node-application-to-cloud-based-kubernetes-deployment-1fed34280952)
- [Access OCI OKE Kubernetes Dashboard locally through Cloud Shell Proxy and ngrok](https://medium.com/oracledevs/access-oci-oke-kubernetes-dashboard-locally-through-cloud-shell-proxy-and-ngrok-43789ea16c22)
- [Oracle Cloud Infrastructure Cloud Shell — integrated OCI CLI, kubectl, terraform, SQL Plus, Docker and Maven](https://medium.com/oracledevs/oracle-cloud-infrastructure-cloud-shell-integrated-oci-cli-kubectl-terraform-sql-plus-docker-c09210e3a535)
- [The Complete Guide To Getting Up And Running With Docker And Kubernetes On The Oracle Cloud](https://recursive.codes/blog/post/1151)
- [Deploy microservices to a Kubernetes cluster](https://docs.oracle.com/en/solutions/deploy-microservices/index.html)
- [Oracle Container Engine for Kubernetes (OKE)](https://enabling-cloud.github.io/oci-learning/manual/OracleContainerEngineForKubernetes.html)
- [Connecting Kubernetes Applications in OKE](https://enabling-cloud.github.io/oci-learning/manual/ConnectingAppsWithServicesOKE.html)
- [OKE Ingress Controller](https://enabling-cloud.github.io/oci-learning/manual/OKEIngressController.html)
- [File Storage Service With Kubernetes (OKE)](https://enabling-cloud.github.io/oci-learning/manual/StaticPersistentVolumeOnOCI.html)
- [Creating Internal (Private) Load Balancer (OKE)](https://enabling-cloud.github.io/oci-learning/manual/CreatingInternalLoadBalancerOKE.html)
- [Single control-plane Kubernetes cluster with kubeadm on OCI](https://enabling-cloud.github.io/oci-learning/manual/KubernetesClusterOnOCI.html)
- [Creating Kubernetes Cluster(OKE) Using Terraform on OCI](https://enabling-cloud.github.io/oci-learning/terraform/OKEClusterUsingTerraform.html)
- [Kubernetes on Oracle Cloud Infrastructure](https://docs.oracle.com/en-us/iaas/Content/Resources/Assets/whitepapers/kubernetes-on-oci.pdf)

### Docker
- [Oracle Cloud Infrastructure (OCI) Container Registry](https://enabling-cloud.github.io/oci-learning/manual/OCIRegistry.html)
- [Adventures in CI/CD #10: Deploying Our Microservice Docker Container To Kubernetes](https://blogs.oracle.com/developers/adventures-in-cicd-10-deploying-our-microservice-docker-container-to-kubernetes)
- [Adventures in CI/CD #9: Deploying A Microservice As A Docker Container](https://blogs.oracle.com/developers/adventures-in-cicd-9%3A-deploying-a-microservice-as-a-docker-container)
- [Oracle Cloud Infrastructure Cloud Shell — integrated OCI CLI, kubectl, terraform, SQL Plus, Docker and Maven](https://medium.com/oracledevs/oracle-cloud-infrastructure-cloud-shell-integrated-oci-cli-kubectl-terraform-sql-plus-docker-c09210e3a535)
- [Run Always Free Docker Container on Oracle Cloud Infrastructure](https://medium.com/oracledevs/run-always-free-docker-container-on-oracle-cloud-infrastructure-c88e36b65610)
- [The Complete Guide To Getting Up And Running With Docker And Kubernetes On The Oracle Cloud](https://recursive.codes/blog/post/1151)
- [Deploy Custom Docker Image To OKE Cluster](https://enabling-cloud.github.io/oci-learning/manual/DeployCustomDockerImageIntoOKECluster.html)

### Terraform
- [Reusable Terraform modules for OCI](https://medium.com/oracledevs/reusable-terraform-modules-for-oci-8fdc9ed1064b)
- [Very first steps in Oracle Cloud Infrastructure as Code with Terraform](https://medium.com/oracledevs/very-first-steps-in-oracle-cloud-infrastructure-as-code-with-terraform-d3179ecc043a)
- [Oracle Cloud Infrastructure Cloud Shell — integrated OCI CLI, kubectl, terraform, SQL Plus, Docker and Maven](https://medium.com/oracledevs/oracle-cloud-infrastructure-cloud-shell-integrated-oci-cli-kubectl-terraform-sql-plus-docker-c09210e3a535)
- [Creating VCN Using Terraform on OCI](https://enabling-cloud.github.io/oci-learning/terraform/CreatingVcnUsingTerraform.html)
- [Creating Bastion Compute Instance Using Terraform on OCI](https://enabling-cloud.github.io/oci-learning/terraform/CreatingBastionComputeInstanceUsingTerraform.html)
- [Terraform module to deploy Palo Alto Networks on Oracle Cloud Infrastructure (OCI)](https://github.com/oracle-quickstart/oci-paloaltonetworks)
- [Terraform module to deploy Fortinet on Oracle Cloud Infrastructure (OCI)](https://github.com/oracle-quickstart/oci-fortinet)
- [Creating Kubernetes Cluster(OKE) Using Terraform on OCI](https://enabling-cloud.github.io/oci-learning/terraform/OKEClusterUsingTerraform.html)

### Microservices
- [MuShop showcase](https://github.com/oracle-quickstart/oci-cloudnative)
- [Microservices Are Easy…](https://recursive.codes/blog/post/1153)
- [Deploy microservices to a Kubernetes cluster](https://github.com/oracle-quickstart/oci-arch-microservice-oke)
- [Adventures in CI/CD #8: Deploying A Microservice With A Tested Persistence Tier In Place](https://blogs.oracle.com/developers/adventures-in-cicd-8-deploying-a-microservice-with-a-tested-persistence-tier-in-place)
- [Adventures in CI/CD #6: Adding A Persistence Tier To Our Microservice](https://blogs.oracle.com/developers/adventures-in-cicd-6-adding-a-persistence-tier-to-our-microservice)
- [Adventures in CI/CD #5: Deploying A Microservice To The Oracle Cloud With GitHub Actions (Gradle Plugin Edition)](https://blogs.oracle.com/developers/adventures-in-cicd-5-deploying-a-microservice-to-the-oracle-cloud-with-github-actions-gradle-plugin-edition)
- [Adventures in CI/CD #4: Deploying A Microservice To The Oracle Cloud With GitHub Actions (OCI CLI Edition)](https://blogs.oracle.com/developers/adventures-in-cicd-4-deploying-a-microservice-to-the-oracle-cloud-with-github-actions-oci-cli-edition)
- [Message Driven Microservices & Monoliths with Micronaut](https://recursive.codes/p/message-driven-microservices-monoliths-with-micronaut-part-1:-installing-kafka-sending-your-first-message)
- [Creating Your First API Gateway In The Oracle Cloud](https://recursive.codes/blog/post/100)
- [Creating A Microservice With Node & SODA - JSON Document Storage In Autonomous DB](https://recursive.codes/blog/post/1135)
- [Microservices From Dev To Deploy, Part 1: Getting Started With Helidon](https://recursive.codes/blog/post/1179)
- [Deploy microservices to a Kubernetes cluster](https://docs.oracle.com/en/solutions/deploy-microservices/index.html)
- [Oracle GoldenGate Microservices Architecture on Oracle Cloud Infrastructure](https://docs.oracle.com/en-us/iaas/Content/Resources/Assets/whitepapers/oracle-goldengate-microservices-architecture-on-oci.pdf)

### Serveless / Functions
- [The Complete Guide To Invoking Serverless Oracle Functions](https://blogs.oracle.com/developers/the-complete-guide-to-invoking-serverless-oracle-functions)
- [Oracle Functions: Serverless On Oracle Cloud - Developer's Guide To Getting Started (Quickly!)](https://recursive.codes/blog/post/1167)
- [Building Cross Platform Native Images With GraalVM](https://blogs.oracle.com/developers/building-cross-platform-native-images-with-graalvm)
- [Simple Serverless Logging For Oracle Functions](https://blogs.oracle.com/developers/simple-serverless-logging-for-oracle-functions)
- [Reliably Requesting REST Results Right From Your SQL Scripts](https://recursive.codes/blog/post/1604)
- [The Complete Guide To Invoking Serverless Oracle Functions](https://recursive.codes/blog/post/1600)
- [Building Cross Platform Native Images With GraalVM](https://recursive.codes/blog/post/1540)
- [Simple Serverless Logging For Oracle Functions](https://recursive.codes/blog/post/1540)
- [Resource Principal Auth With Node.JS For Easy OCI REST API Access From Your Oracle Functions](https://recursive.codes/blog/post/1540)
- [Oracle Functions - Connecting To An ATP Database With A Wallet Stored As Secrets](https://recursive.codes/blog/post/1540)
- [What's New With Notifications And Functions?](https://recursive.codes/blog/post/1384)
- [Oracle Functions: Using Key Management To Encrypt And Decrypt Configuration Variables](https://recursive.codes/blog/post/122)
- [Oracle Functions: Invoking Functions Automatically With Cloud Events](https://recursive.codes/blog/post/124)
- [Oracle Functions: An Easier Way To Talk To Your Autonomous Database](https://recursive.codes/blog/post/126)
- [Oracle Functions - Invoking Functions With The OCI SDK](https://recursive.codes/blog/post/128)
- [Oracle Functions - Invoking Functions With The OCI SDK](https://recursive.codes/blog/post/48)
- [Oracle Functions - Connecting To ATP With Node.JS](https://recursive.codes/blog/post/130)
- [Oracle Functions - Connecting To An ATP Database](https://recursive.codes/blog/post/1133)

### CI/CD
- [Adventures in CI/CD #7: Testing The Persistence Tier With Testcontainers](https://blogs.oracle.com/developers/adventures-in-cicd-7-testing-the-persistence-tier-with-testcontainers)
- [Cloud Cost Cutting: Autoscaling Your Dev/QA Environments](https://blogs.oracle.com/developers/cloud-cost-cutting%3A-autoscaling-your-devqa-environments)
- [Terraform module to deploy Splunk on Oracle Cloud Infrastructure](https://github.com/oracle-quickstart/oci-splunk)
- [Set up a CI/CD pipeline for cloud deployments](https://docs.oracle.com/en/solutions/cicd-pipeline/index.html)


## Frameworks

### Jenkins
- [Terraform module to deploy Jenkins on Oracle Cloud Infrastructure (OCI)](https://github.com/oracle-quickstart/oci-jenkins)
- [Deploy OCI infrastructure using Jenkins pipeline](https://github.com/oracle-quickstart/oci-orm-cicd)
- [Jenkins Up And Running On OCI](https://enabling-cloud.github.io/oci-learning/manual/JenkinsUpAndRunningOnOCI.html)

### Helidon
- [Helodon official docs](https://helidon.io/docs/latest/#/about/01_overview)
- [Migrating a Helidon SE application to Gradle](https://blogs.oracle.com/developers/migrating-a-helidon-se-application-to-gradle)
- [Building And Deploying A Helidon Microservice With Hibernate Part 1](https://recursive.codes/blog/post/1147)
- [Microservices From Dev To Deploy, Part 1: Getting Started With Helidon](https://recursive.codes/blog/post/1179)

### Micronaut
- [Message Driven Microservices & Monoliths with Micronaut](https://recursive.codes/p/message-driven-microservices-monoliths-with-micronaut-part-1:-installing-kafka-sending-your-first-message)
- [Announcing the Micronaut Oracle Cloud Infrastructure Module for Simple SDK Integration and Performant Serverless Functions](https://recursive.codes/blog/post/1580)
- [Oracle Cloud Vault as a Secure, Distributed Config Store for your Micronaut Applications](https://recursive.codes/blog/post/1540)
- [Oracle Universal Connection Pool (UCP) Support Added To Micronaut](https://recursive.codes/blog/post/1540)
- [Easy Messaging With Micronaut's Kafka Support And Oracle Streaming Service](https://recursive.codes/blog/post/100)
- [Microservices The Easy Way With ORDS And Micronaut - Part 1](https://recursive.codes/blog/post/1141)
- [Oracle Cloud Detection And Metadata With Micronaut](https://recursive.codes/blog/post/50)
- [Deploying A Micronaut Microservice To The Cloud](https://recursive.codes/blog/post/1157)

### Kafka
- [Oracle Streaming Service with Kafka Connect](https://blogs.oracle.com/developers/oracle-streaming-service-with-kafka-connect)
- [Terraform module to deploy Kafka on Oracle Cloud Infrastructure](https://github.com/oracle-quickstart/oci-kafka)
- [Using Kafka Connect With Oracle Streaming Service And Autonomous DB](https://recursive.codes/blog/post/92)
- [Easy Messaging With Micronaut's Kafka Support And Oracle Streaming Service](https://recursive.codes/blog/post/100)
- [Migrate Your Kafka Workloads To Oracle Cloud Streaming](https://recursive.codes/blog/post/114)
- [Deploy an Apache Spark cluster in manager/worker mode](https://docs.oracle.com/en/solutions/spark-master-worker-mode/index.html)

### Elasticsearch
- [Deploy Elasticsearch and Kibana](https://docs.oracle.com/en/solutions/deploy-elk/index.html)


### Kibana
- [Deploy Elasticsearch and Kibana](https://docs.oracle.com/en/solutions/deploy-elk/index.html)

### NoSQL
- [REST Data Services SODA for REST Developer's Guide](https://docs.oracle.com/database/ords-18.1/ADRST/getting-started-soda-rest.htm#ADRST-GUID-C67498D7-5783-4969-80EF-C180CEC1144A)
- [Build Swift apps with Oracle’s Autonomous Database and NoSQL](https://medium.com/so-much-code/build-swift-apps-with-oracles-autonomous-database-and-nosql-f1dee7e7cec3)
- [New Offering Launch - Oracle Boosts Developer Velocity with Autonomous JSON Database](http://enswmu.blogspot.com/2020/08/new-offering-launch-oracle-boosts.html)
- [Launch & Persist JSON Documents In The Cloud In 10 Minutes Or Less With Autonomous JSON Database](https://blogs.oracle.com/developers/launch-persist-json-documents-in-the-cloud-in-10-minutes-or-less-with-autonomous-json-database?SC=:so:tw:or:awr:odv::&pcode=&source=:so:tw:or:awr:odv::)

### Flutter
- [Use OCI with Flutter/Dart](https://github.com/oracle-dart/oracle.dart)
- [Node for Oracle Database](https://github.com/oracle/node-oracledb)
- [Oracle database access support for Dart](https://pub.dev/packages/oracle)
